...
export function retrieveJsonObjectFromJsonFile(objectNameKey: string, jsonFileName: string): string {
  const fileData: string = fs.readFileSync('src/resources/testData/services/'+ jsonFileName, 'utf-8');
...
export function saveObjectIntoJsonFile(key: string, newValue: string, jsonFileName: string): void {
  const filePath = `src/resources/testData/services/${jsonFileName}`;
...
export default function replaceDynamicKeyWithActualValues(jsonObject: unknown): unknown {
